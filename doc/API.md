...
1. `JitsiConnection(appID, token, options)` - constructor. Creates the conference object.
    ...
    - `options` - JS object with configuration options for the server connection. You can change the following properties there:
        ...
        2. `bosh` - __DEPRECATED__, use serviceUrl to specify either BOSH or Websocket URL.
        3. `hosts` - JS Object
            - `domain`
            - `muc`
            - `anonymousdomain`
        ...
        6. `enableWebsocketResume` - True/false to control the stream resumption functionality.
        7. `websocketKeepAlive` - The websocket keep alive interval in milliseconds. It's 4 minutes by default with jitter. Pass -1 to disable. The actual interval equation is: jitterDelay = (interval * 0.2) + (0.8 * interval * Math.random()). The keep alive is HTTP GET request to the {@link options.serviceUrl}
        8. xmppPing - (optional) JS Object - xmpp ping options
